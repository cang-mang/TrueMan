...
fn exe() {
    ...
    let tmp = "Client".as_bytes();
    ctx.update(&REQ[..14]).update(&REQ[26..]).update(&tmp[..6])
       .r#final(&mut val);

    assert_eq!(val[ 0], 0x44);
    assert_eq!(val[ 1], 0x5D);
    assert_eq!(val[ 2], 0xA8);
    assert_eq!(val[ 3], 0xD3);
    assert_eq!(val[ 4], 0xB9);
    assert_eq!(val[ 5], 0x88);
    assert_eq!(val[ 6], 0x78);
    assert_eq!(val[ 7], 0x4C);
    assert_eq!(val[ 8], 0x0A);
    assert_eq!(val[ 9], 0xCF);
    assert_eq!(val[10], 0x40);
    assert_eq!(val[11], 0x9B);

    let tmp = "Server".as_bytes();
    ctx.update(&RSP[..10]).update(&RSP[22..]).update(&tmp[..6])
       .r#final(&mut val);

    assert_eq!(val[ 0], 0xA3);
    assert_eq!(val[ 1], 0x71);
    assert_eq!(val[ 2], 0x5C);
    assert_eq!(val[ 3], 0xBB);
    assert_eq!(val[ 4], 0x79);
    assert_eq!(val[ 5], 0xE0);
    assert_eq!(val[ 6], 0xF8);
    assert_eq!(val[ 7], 0x5E);
    assert_eq!(val[ 8], 0x2C);
    assert_eq!(val[ 9], 0xA7);
    assert_eq!(val[10], 0xEC);
    assert_eq!(val[11], 0x45);
}
